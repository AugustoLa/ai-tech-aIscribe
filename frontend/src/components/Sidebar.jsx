...
function Sidebar() {
    ...
    return (
        <div className={`${open ? 'w-60' : 'w-20'} duration-500 h-screen bg-black relative `}>
            ...
            <ul className={`pt-6 flex flex-col items-end ${!open && 'items-center'}`}>
                {Menus.map((menu, index) => (
                    <Link to={menu.to}>
                        <li className={` text-white text-sm flex items-center gap-x-4 cursor-pointer p-2 hover:bg-gray-100/10 rounded-md ${menu.gap ? "mt-9" : "mt-2"}`} key={index}>
