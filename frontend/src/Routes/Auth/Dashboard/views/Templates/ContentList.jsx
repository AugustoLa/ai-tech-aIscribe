...
function ContentList() {
  ...
  const handleSubmit = async () => {
    ...
    const contentList = `Make bullet points with the following content: ${content}`;
    await sendTextToAPI(contentList, `content`);
    setLoading(false);
  };
      ...
      <div className="flex flex-col items-center h-full w-80 text-end xs:mt-5">
        <div className="w-full">
          ...
          <div className="flex flex-col">
            <label className="text-sm text-[#1d2340] pb-2 font-bold">
              Content List Title
            </label>
            <textarea
              className="py-2 md:h-[25rem] sm:h-[15rem] text-sm pl-2 mb-6 border border-[#b8c7e5] bg-white rounded-lg w-full px-2"
              placeholder="7 ways to write better"
              value={content}
              onChange={(e) => setContent(e.target.value)}
            ></textarea>
            <div className="w-full">
              <button
                className={`flex items-center justify-center bg-blue-500 w-full h-10 rounded-md ${!content ? 'opacity-50 cursor-not-allowed' : ''}`}
